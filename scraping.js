...
def main():
    ...
    try:
        ...
        html = fetch_html(url)
        #一時的
        def save_html_to_file(html, filename="ndl_sample.html"):
            try:
                with open(filename, "w", encoding="utf-8") as f:
                    f.write(html)
                full_path = os.path.abspath(filename)
                logging.info(f"[SUCCESS] HTML保存成功: {full_path}")
                print(f"HTMLファイルを保存しました: {full_path}")
            except Exception as e:
                logging.error(f"[ERROR] HTML保存失敗: {e}")
